...
// CREATE OWNERSHIP
//
// Flow (NEW ORDER — important for dev OTP fallback):
// 1. Look up vehicle by plate
// 2. Generate a candidate OTP
// 3. Call sendOTPSms — if dev mode is active, it returns a different OTP value
// 4. Hash WHICHEVER OTP came back from sendOTPSms (not the candidate)
// 5. Insert/update DB with that hash
//
// This guarantees the hash in the DB always matches what the user will type.
// ─────────────────────────────────────────────────────────────────────────────
const createOwnership = async (userId, plateNumber) => {
  ...
  // 3. Generate a CANDIDATE OTP and try to send it
  //    sendOTPSms may override it (dev mode) — its return value is the
  //    OTP that the user will actually type into the app.
  let otp = generateOTP();
  ...
  // 4. Use whichever OTP sendOTPSms confirmed — this is the one we hash
  if (smsResult && smsResult.otp) {
    otp = smsResult.otp;
  }

  // 5. Hash the final OTP and persist
  ...
  return {
    ...
    // Dev mode: expose OTP in the response so the test client can see it.
    // `dev` covers Twilio-not-configured AND DEV_FIXED_OTP modes;
    // `devFallback` covers when Twilio failed and we fell back to fixed OTP.
    ...((smsResult.dev || smsResult.devFallback) && {
      otp,
      dev_note:
        smsResult.devFallback
          ? '⚠️ Twilio failed — fixed dev OTP used. Remove in production.'
          : '⚠️ Dev mode — OTP shown here. Remove in production.',
    }),
  };
};
...
// Same SMS-first / hash-after pattern as createOwnership.
// ─────────────────────────────────────────────────────────────────────────────
