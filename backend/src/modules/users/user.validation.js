...
const createUserSchema = {
  [Segments.BODY]: Joi.object({
    ...
    national_id: Joi.string().trim().pattern(/^\d{14}$/).required().messages({
...
// ── Manual validation for multipart/form-data registration ───────────────────
// celebrate/Joi cannot validate req.body from multipart requests directly
// because the body isn't parsed as JSON. We run Joi manually after multer.
const validateCreateUser = async (req, res, next) => {
  try {
    const schema = createUserSchema[Segments.BODY];
